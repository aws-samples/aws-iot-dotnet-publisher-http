...
Write-Output "Creating thing: aws-iot-dotnet-publisher-http-framework.."
$ProvisioningTemplate = Get-Content -Path "$ScriptDirectory\provisioning_template.json" -Raw
Register-IOTThing `
    -TemplateBody $ProvisioningTemplate `
    -Parameter @{ "ThingName"="aws-iot-dotnet-publisher-http-framework"; "CertificateId"="$CertificateId" } | Out-Null
